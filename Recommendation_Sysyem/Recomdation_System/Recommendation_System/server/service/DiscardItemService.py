...
class DiscardItemService:
    ...
    def view_discard_list(self, request, client_socket):
        try:
            ...
            cursor.execute("""
                SELECT mi.id, mi.name, mi.average_rating, GROUP_CONCAT(f.comment SEPARATOR ', ') as sentiments
                FROM menu_items mi
                LEFT JOIN feedback f ON mi.id = f.menu_item_id
                WHERE mi.average_rating < 2 OR f.sentiment = 'negative'
            ...
            return {'status': 'error', 'message': 'Failed to retrieve discard list'}

    def remove_menu_item(self, request, client_socket):
        if request.get('role') not in [1, 2]:  # Admin or Chef
